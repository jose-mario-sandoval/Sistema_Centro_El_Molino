...
3. Agregar, al final del mismo `describe` (reutiliza el `base()` ya declarado en su nivel superior,
   sin volver a declararlo), un bloque anidado nuevo para el pedido libre:
...
1. **Primero, el fixture compartido** (línea 6): `VALIDO` tiene `tipo: 'charla'`, una categoría que
   deja de existir en la Tarea 2. Casi todos los tests del archivo hacen `...VALIDO` sin pisar `tipo`
   (incluidos varios que llaman `esquemaEvento.parse(...)` directo, que tira si `tipo` es inválido) —
   si no se corrige esto primero, todo el archivo queda roto, no solo los tests de tipo. Cambiar a:
   `const VALIDO = { titulo: 'Charla formativa', fecha: '2026-09-16', hora: '19:30', tipo: 'san_gabriel', requiere_cocina: [] }`.
2. En `describe('esquemaEvento: tipo y pedidos a la cocina', ...)`, cambiar
