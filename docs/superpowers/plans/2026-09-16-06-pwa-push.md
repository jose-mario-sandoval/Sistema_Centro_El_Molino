...
Igual, inmediatamente antes del `return exito({ id })` de `responderMensaje`:
...
```bash
grep -n "after(" "app/(app)/mensajes/acciones.ts"
npm run typecheck && npm run lint && npm test
```

Esperado: dos líneas con `after(` (una por acción, cada una justo antes de un `return exito(`); sin errores de tipos ni lint; unitarias en verde.
