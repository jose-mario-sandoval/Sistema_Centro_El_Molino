...
**Arquitectura:**
- **Parte A (PR de esquema):** tablas `suscripciones_push` y `avisos_enviados` con RLS, función `llamar_recordatorios()` que lee URL y secreto de Supabase Vault y hace POST con `pg_net`, y job de `pg_cron` cada 5 minutos.
...
7. El interruptor "Recordatorio de hora límite" solo se muestra a Director y Residente (Administración no recibe recordatorios, spec §8.2).
...
supabase/migrations/<ts>_push.sql                  tablas, RLS, pg_net, llamar_recordatorios, job de pg_cron
...
### Tarea 3: Migración — `pg_net`, `llamar_recordatorios()` y job de `pg_cron`

**Archivos:**
- Modificar: `supabase/migrations/<timestamp>_push.sql` (agregar al final)

- [ ] **Paso 1: Agregar extensiones, función y job al final de la migración**
...
```bash
git add supabase/migrations
git commit -m "feat(db): llamar_recordatorios con Vault y pg_net, job de pg_cron cada 5 minutos"
```
...
Esperado: `calidad` y `base-de-datos` en verde. En el log de `base-de-datos`, la migración `<ts>_push.sql` se aplica en `supabase start` y `tests/integration/push.test.ts` pasa (14 pruebas). Si falla: `gh run view --log-failed`, corregir, commit y `git push`.
...
Esperado: `calidad` y `base-de-datos` en verde, incluidas `tests/e2e/push.spec.ts` (6 pruebas) y las e2e de las demás pistas (login y cierre de sesión, mensajes, configuraciones). Si falla: `gh run view --log-failed`, corregir, commit y `git push`.
...
Esperado: corridas con `status = succeeded` y respuestas con `status_code = 202`. Un `401` indica que `cron_secret` no coincide con `CRON_SECRET` de Vercel; un `404`, que `url_app` es incorrecta: volver a ejecutar el snippet con los valores correctos.
