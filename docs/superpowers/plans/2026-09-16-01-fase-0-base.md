...
[auth.email]
# La plantilla trae true; debe quedar false explícito (spec §10).
enable_signup = false
enable_confirmations = false
```

Verificar que `[auth]` y `[auth.email]` contengan `enable_signup = false`:

```bash
grep -n "enable_signup" supabase/config.toml
```

Esperado: dos líneas, ambas con `false`.
...
### Tarea 7: Aplicar la migración al proyecto (excepción de la Fase 0) y generar tipos

> Esta es la **única** vez que se aplica una migración desde una computadora (spec §10). Después, solo la Action de despliegue.

**Archivos:**
- Crear: `lib/supabase/database.types.ts` (generado)

- [ ] **Paso 1: Iniciar sesión en la CLI**

```bash
npx supabase login
```

Esperado: abre el navegador y la CLI queda autenticada (`You are now logged in`).

- [ ] **Paso 2: Vincular el proyecto**

El *project ref* es el subdominio de `NEXT_PUBLIC_SUPABASE_URL` (por ejemplo `abcdefghijkl` en `https://abcdefghijkl.supabase.co`).

```bash
npx supabase link --project-ref <project-ref>
```

Ingresar la contraseña de base de datos cuando la pida. Esperado: `Finished supabase link.`

- [ ] **Paso 3: Ver qué se va a aplicar**

```bash
npx supabase db push --dry-run
```

Esperado: lista solo `<timestamp>_base.sql`.
...
Esperado: `Finished supabase db push.` Si falla por conexión (IPv6), repetir con la cadena del *Session pooler* (Project Settings → Database → Connection string → Session pooler):
`npx supabase db push --db-url "postgresql://postgres.<ref>:<contraseña>@<host-pooler>:5432/postgres"`.

- [ ] **Paso 5: Aplicar la configuración de Auth**

```bash
npx supabase config push
```

Revisar el diff que muestra (debe incluir `enable_signup = false`) y confirmar con `y`. Esperado: `Finished supabase config push.`

- [ ] **Paso 6: Generar tipos**
...
Esperado: `lib/supabase/database.types.ts` contiene `perfiles`, `horas_limite`, `rol`, `tiempo_comida`, `estado_comida`, `origen_seleccion`, `mi_rol`, `soy_activo`, `zona_horaria_app`.

```bash
grep -c "horas_limite\|perfiles\|zona_horaria_app" lib/supabase/database.types.ts
```
...
git commit -m "chore(db): tipos generados del esquema base"
```
...
export async function crearCuenta(admin: SupabaseClient<Database>, datos: DatosCuenta): Promise<ResultadoCrearCuenta> {
  ...
  if (error || !data.user) {
    const repetido = error?.code === 'email_exists' || error?.status === 422
...
### Tarea 18: Despliegue a producción

**Archivos:**
- Crear: `vercel.json`, `.github/workflows/desplegar.yml`
...
- [ ] **Paso 2: `.github/workflows/desplegar.yml`**

La Action se salta sola mientras el dueño no configure la variable `SUPABASE_PROJECT_REF` (spec Anexo A.2).
...
jobs:
  desplegar:
    ...
    steps:
      ...
      - name: Traer configuración de Vercel
        run: npx vercel@59 pull --yes --environment=production --token="$VERCEL_TOKEN"
      - name: Build
        run: npx vercel@59 build --prod --token="$VERCEL_TOKEN"
      - name: Desplegar
        run: npx vercel@59 deploy --prebuilt --prod --token="$VERCEL_TOKEN"
```

- [ ] **Paso 3: Commit**

```bash
git add vercel.json .github/workflows/desplegar.yml
git commit -m "ci: despliegue de producción (migraciones antes que código)"
```
...
| `npm run db:tipos` | Regenera `lib/supabase/database.types.ts` desde el proyecto |
...
- Migraciones con `npx supabase migration new <nombre>`; **nadie ejecuta `supabase db push`**: las aplica la Action al mergear.
...
Esperado: CI en verde. Pedir revisión al otro colaborador. **Al mergear**:
- si el dueño ya configuró los secretos, la Action de despliegue aplica la migración (ya aplicada, no hace nada) y despliega;
- si no, la Action se salta.
